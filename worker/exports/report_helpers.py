"""
Shared computation helpers for Excel and HTML/Markdown report exporters.
Covers: cookie register, tag source attribution, performance stats, executive metrics.
"""
...
# ---------------------------------------------------------------------------
# Known cookie classification dictionary
# (name → (vendor_name_or_None, purpose_category, description))
# ---------------------------------------------------------------------------
...
# Prefix-based classification (checked if exact name not found)
...
def _classify_cookie(name: str) -> tuple:
    """Return (vendor_name, purpose_category, description) for a cookie name."""
    # Exact match
...
def _extract_base_domain(url: str) -> str:
    """Return the registrable domain from a URL (e.g. 'example.com')."""
    try:
...
def _parse_expiry(expires) -> str:
    """Convert a Unix timestamp (or -1) to a human-readable expiry string."""
...
# ---------------------------------------------------------------------------
# Gap 2 — Cookie Register
# ---------------------------------------------------------------------------

def build_cookie_register(pages) -> List[Dict[str, Any]]:
    """
    Aggregate cookies_detail across all PageVisit records into a deduplicated
    cookie register with classification and first/third-party status.
    """
    ...
    for page in pages:
        ...
        if page.cookies_detail and isinstance(page.cookies_detail, list):
            ...
            # Fall back to names-only; wrap as minimal dicts
    ...
    # Sort: purpose category priority, then party (1st first), then name
...
def build_tag_attribution(vendors, pages, requests) -> Dict[str, str]:
    """
    For each audit-level vendor, determine how its scripts are loaded.
    Returns {vendor_key: load_source_string}.
    """
    # Detect GTM presence
    gtm_present = False
    for req in requests:
        url = (req.url or "").lower()
        if any(ind in url for ind in GTM_INDICATORS):
            gtm_present = True
            break

    # Build set of all script_srcs across pages (lowercased)
    all_script_srcs: set = set()
    for page in pages:
        for src in (page.script_srcs or []):
            all_script_srcs.add(src.lower())

    # Load vendor signatures to know each vendor's domains
    try:
...
# ---------------------------------------------------------------------------
# Gap 5 — Performance Impact
# ---------------------------------------------------------------------------

def build_performance_stats(vendors, requests, page_level_vendors) -> List[Dict[str, Any]]:
    """
    Compute per-vendor performance metrics using NetworkRequest.timing_ms.
    page_level_vendors: list of DetectedVendor with page_visit_id set (used for vendor_id → key mapping).
    """
    # Map page-level vendor id → vendor_key
    pvid_to_key: Dict = {}
    for pv in page_level_vendors:
        pvid_to_key[pv.id] = pv.vendor_key

    # Group requests by vendor_key via vendor_id FK
    keyed_reqs: Dict[str, List] = defaultdict(list)
    unmatched_domains: Dict[str, List] = defaultdict(list)

    for req in requests:
        if req.vendor_id and req.vendor_id in pvid_to_key:
            keyed_reqs[pvid_to_key[req.vendor_id]].append(req)
        elif req.is_tracking_related:
            try:
                domain = urlparse(req.url).netloc if req.url else "unknown"
                unmatched_domains[domain].append(req)
            except Exception:
                pass
    ...
    for vendor in vendors:
        ...
        pages = len(set(str(r.page_visit_id) for r in reqs)) if reqs else vendor.page_count or 0
    ...
    # Sort by avg timing descending (None last)
    stats.sort(key=lambda x: (-(x["avg_timing_ms"] or 0), x["vendor_name"]))
    return stats


# ---------------------------------------------------------------------------
# Gap 7 — Executive Metrics
# ---------------------------------------------------------------------------

def compute_executive_metrics(vendors, issues, config, pages) -> Dict[str, Any]:
    """
    Compute executive-level risk rating, compliance posture, and top findings.
    """
    issue_counts = {
    ...
    if not has_tracking:
        gdpr_posture = "N/A — no tracking detected"
        ccpa_posture = "N/A — no tracking detected"
